function [comp_img, uncomp_img] = lossy_haar(img_)
    img = img_;
    H = zeros(8,8);
    delta = 1; % Only greater than delta pixel differences are accepted, otherwise zero.
    ...
    for r=1:b/8  % Dividing image matrix into blocks of 8x8 to perform compression
        for c=1:a/8
            ...
            for i=1:8
                for j=1:8
                    if abs(A(i,j)) <= delta
                        comp_img(i+8*(c-1),j+8*(r-1)) = 0;
                        A(i,j) = 0;
                    else
                        comp_img(i+8*(c-1),j+8*(r-1)) = A(i,j);
                    end
                end
            end
            A = HT_inv*A*H_inv;
            for i=1:8 % Decompressing and updating the compressed image martix
                for j=1:8
                    uncomp_img(i+8*(c-1),j+8*(r-1)) = A(i,j);
                end
            end
        end
    end
    imwrite(uncomp_img,"comp_img_lossy.jpg") % Saving image file locally
    comp_name = "comp_img_lossy.jpg";
